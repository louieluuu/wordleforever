  ...
  return (
    <>
      {gameMode.includes("offline") ? null : (
        <div style={{ display: "flex", alignItems: "center", fontSize: "3rem" }}>
          {streak === 0 ? "" : gameMode === "online-private" ? (isInGame ? "" : streak) : streak}
          <Flame className={getStreakClassName()} style={{ width: "3rem", height: "3rem" }} />
        </div>
      )}
    </>
  )
